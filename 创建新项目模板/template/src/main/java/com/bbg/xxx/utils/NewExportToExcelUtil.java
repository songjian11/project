...
public class NewExportToExcelUtil {
     ...
     * @param list   要导出的数据集合
     * @param fieldMap 中英文字段对应Map,即要导出的excel表头 key为实体属性，值是中文名称
     * @return
     */
    public static <T> Integer export(String excelName, List<T> list, LinkedHashMap<String, String> fieldMap, HttpServletResponse response){
        ...
        try {
            // 设置response头信息
            response.reset();
            response.setContentType("application/ms-excel"); // 改成输出excel文件
            response.setHeader("Content-disposition", "attachment; filename="
                    +new String(excelName.getBytes("gb2312"), "ISO-8859-1")  + ".xlsx");

            //创建一个WorkBook,对应一个Excel文件  值1000  在内存中保留1000行，超过行将被刷新到磁盘
            SXSSFWorkbook wb=new SXSSFWorkbook(1000);
            //当临时文件过大时，可使用setCompressTempFiles方法进行压缩，
     ...
     *
     */
    public static <T> void fillSheet(Sheet sheet, List<T> list,
                                     LinkedHashMap<String, String> fieldMap,CellStyle style) throws Exception {
        // 定义存放英文字段名和中文字段名的数组
